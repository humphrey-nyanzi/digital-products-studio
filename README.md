# Digital Products Studio

> Building practical, reliable digital products that solve real operational problems.

---

## Overview

This repository contains the operating system for the Digital Products Studio.

The Studio is a structured product engineering environment for designing, building, releasing and maintaining lightweight digital products.

Rather than treating every new product as an isolated project, the Studio provides a shared set of principles, standards, templates and workflows that ensure every product is developed consistently.

The Studio itself is a product.

Its purpose is to make building future products repeatable.

---

# What We Build

The Studio focuses on practical digital products such as:

* Google Sheets systems
* Microsoft Excel templates
* Google Forms workflows
* Google Apps Script automations
* Dashboards
* Operational reporting tools
* Data collection systems
* Documentation toolkits
* Personal productivity systems

Products are designed to solve one operational problem exceptionally well before expanding in scope.

---

# Product Categories

Current product categories include:

* Sports Competitions
* Sports Business
* Sports Performance & Sports Science
* Personal Productivity & Fitness
* Other Operational Systems

---

# Studio Principles

Every Studio product should:

* Solve a real problem.
* Be practical rather than impressive.
* Remain simple for users.
* Hide unnecessary complexity.
* Be professionally documented.
* Be modular.
* Be easy to customise.
* Work for a global audience where practical.
* Be maintainable over time.

Quality is preferred over quantity.

---

# Repository Structure

```text
digital-products-studio/
│
├── studio/
│   ├── governance/
│   ├── standards/
│   ├── templates/
│   ├── reference/
│   └── playbooks/
│
├── products/
│
├── shared/
│
├── archive/
│
├── README.md
└── .gitignore
```

---

# Studio Documentation

## Governance

Defines why the Studio exists and how it operates.

* Studio Charter
* Studio Manual

---

## Standards

Defines mandatory Studio rules.

Examples:

* Documentation Standards
* Design System
* Release Standards
* Technology & Infrastructure

---

## Templates

Reusable starting points for products and documentation.

Examples:

* Product Template
* ADR Template
* QA Report Template

---

## Reference

Shared Studio knowledge.

Examples:

* Product Vocabulary
* Brand System

---

## Playbooks

Operational workflows.

Examples:

* AI Collaboration Guide
* GitHub Workflow
* Product Development Playbook
* Product Release Playbook

---

# Development Workflow

Every product follows the same lifecycle.

```text
Idea

↓

Research

↓

Requirements

↓

Planning

↓

Design

↓

Development

↓

Internal QA

↓

Packaging

↓

Release Candidate

↓

Public Release

↓

Maintenance

↓

Iteration
```

Detailed guidance is available in the Product Development Playbook.

---

# GitHub Workflow

The Studio manages work using:

* Issues
* Projects
* Labels
* Milestones
* Pull Requests
* Releases

Every significant piece of work should begin as a GitHub Issue.

The GitHub Project represents the Studio pipeline rather than a simple task board.

---

# AI Collaboration

AI is used throughout the Studio to accelerate development.

AI contributors assist with:

* research
* documentation
* implementation
* testing
* review

Humans remain responsible for:

* product direction
* quality
* architectural decisions
* release approval

The AI Collaboration Guide defines the expected workflow.

---

# Current Status

Studio Version:

**v1.0.0**

Current Focus:

Commercialising Product 001: Football Competition Results & Standings Manager through public managed fulfilment, pricing, payment qualification, packaging and measured market validation.

---

# Contributing

Every contributor should begin by reading:

1. Studio Charter
2. Studio Manual
3. GitHub Workflow
4. Product Development Playbook

These documents establish the Studio's operating principles before product work begins.

---

# Repository Philosophy

This repository does not primarily contain products.

It contains the system used to create products.

Individual products should have their own repositories once they reach active development.

The Studio repository remains the shared foundation that every product inherits.

---

# Versioning

The Studio follows semantic versioning where appropriate.

Major changes to Studio standards, workflows or governance should be documented through Architecture Decision Records (ADRs) and reflected in release notes.

---

# Licence

To be determined.

---

# Contact

Studio Director:

Humphrey Nyanzi

---

> Build products that deserve to exist.
