 ...
 * Apps Script automation v0.1.5.0
 */
const PRODUCT001 = {
  setupSheet: 'Setup',
...
function PRODUCT001_applyMatchLifecycleUpgrade_() {
  ...
  review.getRange(4, officialStatusColumn, 1, 2).setNotes([[
    'Manager-only. This is the competition outcome used to update Fixtures and official records. An abandoned report defaults to Postponed, which means reschedule it before opening the Form again.',
    'Manager-only. Use this only when Competition Outcome is Walkover. Choose Home team or Away team. No Team ID is required.'
  ]]);

  const outcomeValidation = SpreadsheetApp.newDataValidation()
    .requireValueInList(['Played', 'Walkover', 'Postponed', 'Void'], true)
    .setAllowInvalid(false)
    .setHelpText('Manager-only. Choose the official competition outcome. An abandoned report normally becomes Postponed unless the competition rules determine another outcome.')
    .build();
  ...
  for (let row = 5; row <= 204; row++) {
    ...
    eligibilityFormulas.push([`=IF(D${row}="","",IF(AND(R${row}="Approved Current",OR(AB${row}="Played",AB${row}="Walkover")),"Yes","No"))`]);
    checkFormulas.push([`=IF(D${row}="","",IF(AND(R${row}="Approved Current",AB${row}=""),"Select competition outcome",IF(AND(R${row}="Approved Current",AB${row}="Played",OR(X${row}="",Y${row}="")),"Official score required",IF(AND(R${row}="Approved Current",AB${row}="Walkover",AD${row}=""),"Walkover missing winner",IF(AND(R${row}="Approved Current",COUNTIFS($D$5:$D$204,D${row},$R$5:$R$204,"Approved Current")>1),"Multiple Approved Current",IF(AND(OR(R${row}="Pending Review",R${row}="Needs Clarification",R${row}="Approved Current"),SUM(COUNTIFS($D$5:$D$204,D${row},$R$5:$R$204,{"Pending Review","Needs Clarification","Approved Current"}))>1),IF(ROW()=MAX(FILTER(ROW($D$5:$D$204),$D$5:$D$204=D${row},(($R$5:$R$204="Pending Review")+($R$5:$R$204="Needs Clarification")+($R$5:$R$204="Approved Current"))>0)),"Multiple submissions - newest","Multiple submissions - earlier"),"OK"))))))`]);
    officialStatusFormulas.push([`=IF($A${row}="","",IFERROR(INDEX(FILTER('Result Review'!$AB$5:$AB$204,'Result Review'!$D$5:$D$204=$A${row},'Result Review'!$R$5:$R$204="Approved Current"),1),""))`]);
    officialWinnerResultFormulas.push([`=IF($A${row}="","",IFERROR(INDEX(FILTER('Result Review'!$AD$5:$AD$204,'Result Review'!$D$5:$D$204=$A${row},'Result Review'!$R$5:$R$204="Approved Current"),1),""))`]);
  }
  ...
  reports.getRange('B4').setValue('Official Results');
  reports.getRange('B5').setFormula('=COUNTIF(\'Result Review\'!$Z$5:$Z$204,"Yes")');
  reports.getRange('D4').setValue('Scheduled for Submission');
  const validation = PRODUCT001_getSheet_('Validation');
  validation.getRange('C14').setValue('Approved result rows pass the manager review checks');
  validation.getRange('D14').setFormula('=IF(COUNTIFS(\'Result Review\'!$D$5:$D$204,"<>",\'Result Review\'!$AA$5:$AA$204,"<>OK")=0,"OK","Issue")');
  validation.getRange('E14').setFormula('=IF(D14="OK","","Check Result Review for an outcome, official score, walkover winner or duplicate submission issue")');
  validation.getRange('C15').setValue('Approved walkovers have an awarded side');
  validation.getRange('D15').setFormula('=IF(COUNTIFS(\'Result Review\'!$AB$5:$AB$204,"Walkover",\'Result Review\'!$AD$5:$AD$204,"",\'Result Review\'!$R$5:$R$204,"Approved Current")=0,"OK","Issue")');
  validation.getRange('E15').setFormula('=IF(D15="OK","","Choose Home team or Away team in Result Review for the approved walkover")');
}
...
function PRODUCT001_rescheduleSelectedFixture() {
  ...
  reviewValues.forEach(function(row, index) {
    if (String(row[3] || '').trim() === matchId && row[17] === 'Approved Current' && row[27] === 'Postponed') {
      currentRows.push(index + 5);
    }
  });
  if (currentRows.length !== 1) {
    ui.alert('Reschedule stopped. This fixture needs exactly one Approved Current Postponed report in Result Review. Review its history before reopening the fixture.');
    return;
  }
  ...
  review.getRange(reviewRow, 18).setValue('Approved Superseded');

  const synced = PRODUCT001_syncResultFormFixtures(false);
  ui.alert('Fixture ' + matchId + ' has been rescheduled and reopened. The earlier postponement remains in Result Review as Approved Superseded. Form sync now has ' + synced + ' eligible fixture(s).');
}
...
function PRODUCT001_applyApprovedFixtureStatuses_() {
  ...
  reviewValues.forEach(function(row) {
    ...
    if (matchId && reviewStatus === 'Approved Current' && competitionOutcome) {
...
function PRODUCT001_showQaStatus() {
  ...
  const message = [
    'Official Results: ' + reports.getRange('B5').getDisplayValue(),
    'Scheduled for Submission: ' + reports.getRange('D5').getDisplayValue(),
...
function PRODUCT001_buildCanonicalFormItems_(form) {
  ...
  const statusUpdatePage = PRODUCT001_getOrCreatePageBreakItem_(form, 'Match Status Update')
    .setHelpText('Explain why the fixture was postponed or abandoned. A postponed fixture is rescheduled by the competition manager. An abandoned match is reviewed before a competition outcome is recorded.');
