...
1. Customer-copy navigation, Form and response isolation passed.
2. v0.1.5.9 and its four-scope manifest passed ordinary-user operation.
3. The owned domain, support route, public User Manual, product homepage and privacy policy are live.
4. The standard Cloud project, OAuth consent identity, branding and data-access verification are approved.
...
6. The buyer owns the workbook, Result Form and competition data. Studio access is removed unless temporary support access is explicitly requested.

The target delivery promise is within one business day after a complete eligible order. The first public offer does not support ownership delivery to work, school or organisation-managed Google accounts.
...
Do not blank or distribute the working master. Preserve v0.1.5.9, the Result Form, the four approved scopes, OAuth client and standard Cloud project unless a commercial requirement exposes a workflow-impact reason. Keep v0.1.6.0 uninstalled until such a reason exists. Do not repeat full operational QA without a relevant implementation change.
