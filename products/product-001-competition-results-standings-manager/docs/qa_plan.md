...
| Least-privilege authorisation | Install v0.1.5.8 Code.gs and manifest in the master, create a disposable copy and reauthorise | The copied project inherits the hidden manifest without customer editing. Consent requests current workbook access, Forms management and trigger management only. Full Drive and account-wide Sheets access are absent. |
